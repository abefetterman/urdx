...
function boxInertiaTensor(dx, dy, dz, m) {
  return {
    ixx: (0, _utils.truncate)(m * (Math.pow(dy, 2) + Math.pow(dz, 2)) / 12.0),
    ixy: 0,
    ixz: 0,
    iyy: (0, _utils.truncate)(m * (Math.pow(dx, 2) + Math.pow(dz, 2)) / 12.0),
    iyz: 0,
    izz: (0, _utils.truncate)(m * (Math.pow(dx, 2) + Math.pow(dy, 2)) / 12.0)
  };
}
