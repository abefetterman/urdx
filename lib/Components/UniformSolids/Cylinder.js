...
function cylinderInertiaTensor(r, h, m) {
  return {
    ixx: (0, _utils.truncate)(m * (3 * Math.pow(r, 2) + Math.pow(h, 2)) / 12.0),
    ixy: 0,
    ixz: 0,
    iyy: (0, _utils.truncate)(m * (3 * Math.pow(r, 2) + Math.pow(h, 2)) / 12.0),
    iyz: 0,
    izz: (0, _utils.truncate)(m * Math.pow(r, 2) / 2.0)
  };
}
